...
pub struct RegexHintProcessorData {
    pub code: Code,
...
impl RegexHintProcessor {
    ...
    pub fn add_hint(&mut self, hint_code_regex: String, func: Rc<HintFunc>) {
...
impl HintProcessor for RegexHintProcessor {
    fn execute_hint(
        ...
        let hint_data = hint_data
            .downcast_ref::<RegexHintProcessorData>()
        ...
        match &hint_data.code {
            ...
            Code::Function(name, args) => {
                let ptr_hint_func = self.hints.get(name).ok_or(VirtualMachineError::WrongHintData)?;
                ptr_hint_func.0(vm, exec_scopes, &hint_data.ids_data.clone(), &hint_data.ap_tracking.clone(), constants, args)?
            }
        };
    ...
    fn compile_hint(
        ...
        let index_of_opening_parenthesis = match hint_code_trim.find('(') {
            None =>
                return Ok(any_box!(RegexHintProcessorData {
        ...
        Ok(any_box!(RegexHintProcessorData {
